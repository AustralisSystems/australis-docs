...
## 🏗️ THREE-TIER HIERARCHICAL STRUCTURE

### Overview

The instruction system follows a **three-tier hierarchical structure** that establishes clear precedence, relationships, and execution order:
...
### Structure Order and Precedence

**Precedence Rule**: Higher tiers govern lower tiers. When conflicts occur:
1. **DOCTRINE** takes precedence over PROTOCOLS and INSTRUCTIONS
2. **PROTOCOLS** take precedence over INSTRUCTIONS
3. **INSTRUCTIONS** execute within the constraints of DOCTRINE and PROTOCOLS

**Execution Order**:
1. **LOAD_AND_PARSE**: DOCTRINE → PROTOCOLS (foundational rules loaded first)
2. **EXECUTE**: INSTRUCTIONS (execution workflows run after foundational rules are understood)

### Tier 1: DOCTRINE

**Definition**: The foundational execution rules that define **HOW** the system must operate, think, prioritize, and constrain itself.

**Purpose**:
- Establishes the core operational philosophy
- Defines behavioral constraints and principles
- Governs ALL downstream prompts and payloads
- Sets non-negotiable execution standards

**Characteristics**:
- **Single File**: Only one DOCTRINE file exists (000-DOCTRINE)
- **Non-Negotiable**: Requirements are absolute and cannot be overridden
- **Universal Application**: Applies to ALL execution contexts
- **Behavioral Focus**: Defines HOW to operate, not WHAT to do

**Key Principles** (from Enterprise Canonical Execution Doctrine):
- **Log-First Doctrine**: Logs are primary source of truth
- **Evidence-Based**: No guessing, speculation, or hand-waving
- **Minimal Changes**: Prefer smallest change that resolves issue
- **Production Code Mandate**: 100% implementation, 0 errors/warnings/issues
- **Implementation Consistency**: Similar features MUST have similar implementation completeness
- **No Deprecation Policy**: Complete refactoring/rewriting only, no shims or partial solutions
- **Universal Container Deployment**: Must work across ALL container platforms
- **Cybersecurity & AI Security**: Highest standards compliance required
- **Python 3.12+ Requirement**: Minimum version enforcement
- **Dependency Management**: Latest stable versions, immediate updates required

**File Naming**: `000-DOCTRINE-{Name}-{version}.yaml`

**Example**: `000-DOCTRINE-Enterprise_Canonical_Execution-v2.0.1.yaml`

### Tier 2: PROTOCOLS

**Definition**: Operational procedures and validation/compliance rules that define **HOW** to validate, remediate, and ensure compliance.

**Purpose**:
- Defines validation methodologies
- Establishes compliance checkpoints
- Provides operational procedures
- Enforces quality standards

**Characteristics**:
- **Multiple Files**: One to many PROTOCOL files (001-099 range)
- **Domain-Specific**: Each protocol covers a specific domain (validation, remediation, code quality, etc.)
- **Enforcement Rules**: Defines HOW to enforce standards
- **Reference DOCTRINE**: Protocols reference and enforce DOCTRINE principles

**Key Protocol Categories**:
- **Golden Rule Execution**: Mandatory 7-step workflow with progressive memory queries
- **Zero Tolerance Remediation**: Enhanced violation detection, pre-flight scans, interface completeness
- **Production Code Quality**: 12 mandatory metrics, validation commands, remediation strategies
- **Test Implementation**: Test types, quality standards, enterprise templates
- **MCP Tools Workflow**: Tool usage recommendations and patterns
- **RFC 2119 Requirements Language**: Standard terminology (MUST, SHALL, SHOULD, MAY, FORBIDDEN, PROHIBITED)
- **Digital Assistant Roles**: Centralized role definitions with versioning and inheritance

**Enhanced Features** (from recent updates):
- **Violation Detection Patterns**: Comprehensive pattern matching for incomplete code
- **Pre-Flight Scans**: Mandatory violation scanning before starting work
- **Interface Completeness Checks**: Function signature/implementation verification
- **Semantic Completeness**: Code completeness verified semantically, not just keyword search
- **Pattern Consistency Mandate**: Similar features must match implementation completeness
- **File Modification Checkpoints**: Scan before/after file modifications
- **Post-Modification Validation**: Re-scan modified files for violations

**File Naming**: `{001-099}-PROTOCOL-{Name}-{version}.yaml`
...
**Definition**: Executable workflows and task definitions that specify **WHAT** to do.

**Purpose**:
- Defines specific execution workflows
- Provides step-by-step task guidance
- Implements DOCTRINE and PROTOCOL requirements
- Executes actual work

**Characteristics**:
- **Multiple Files**: One or more INSTRUCTION files (100-199, 200-299 ranges)
- **Executable**: Defines workflows that are actually executed
- **Task-Specific**: Each instruction covers a specific task or workflow
- **Enforce PROTOCOLS**: Instructions enforce and reference PROTOCOLS
- **Reference DOCTRINE**: Instructions operate within DOCTRINE constraints

**Instruction Categories**:
- **100-199 Range**: Formal multistep instructions (complete workflows)
  - Example: `104-INSTRUCTIONS-Execute_Implementation_Phase_Tasks-v2.0.0.yaml`
  - Example: `106-INSTRUCTIONS-Validate_Code_Quality_and_Compliance-v2.0.0.yaml`
  - Example: `107-INSTRUCTIONS-Remediate_And_Refactor_Codebase-v2.0.0.yaml`

- **200-299 Range**: Focused instructions (targeted, single-purpose tasks)
  - Example: `202-INSTRUCTIONS-Pure_Code_Implementation_Execution_Protocol-v2.0.0.yaml`
  - Example: `203-INSTRUCTIONS-FastAPI_Design_Implementation_Refactor-v2.0.0.yaml`

**Enhanced Features** (from recent updates):
- **Incomplete Code Pattern Detection**: Mandatory violation scanning as part of validation
- **Mandatory Violation Pattern Detection**: Required before marking work complete
- **Pattern Consistency Requirements**: Verify consistency with existing implementations
- **Interface/Implementation Mismatch Detection**: Function signature verification
- **Detection Pattern Examples**: Clear examples of violations vs acceptable patterns

**File Naming**: `{100-299}-INSTRUCTIONS-{Name}-{version}.yaml`
...
**DOCTRINE → PROTOCOLS**:
- PROTOCOLS reference and enforce DOCTRINE principles
- PROTOCOLS provide domain-specific implementation of DOCTRINE rules
- PROTOCOLS cannot contradict DOCTRINE (DOCTRINE takes precedence)

**PROTOCOLS → INSTRUCTIONS**:
- INSTRUCTIONS enforce and reference PROTOCOLS
- INSTRUCTIONS implement PROTOCOL procedures
- INSTRUCTIONS cannot contradict PROTOCOLS (PROTOCOLS take precedence)

**DOCTRINE → INSTRUCTIONS**:
- INSTRUCTIONS operate within DOCTRINE constraints
- INSTRUCTIONS implement DOCTRINE principles
- INSTRUCTIONS cannot contradict DOCTRINE (DOCTRINE takes precedence)

### Concatenation and Reference Model

**Subpart Referencing**:
- YAML files are structured to allow subpart referencing using dot notation
- Example: `protocol.note_violation_detection.patterns.explicit_violations`
- Enables precise referencing of specific sections

**Cumulative Application**:
- DOCTRINE principles apply to all execution
- PROTOCOLS concatenate with DOCTRINE (PROTOCOLS + DOCTRINE)
- INSTRUCTIONS concatenate with PROTOCOLS and DOCTRINE (INSTRUCTIONS + PROTOCOLS + DOCTRINE)

**Example Flow**:
```
1. Load DOCTRINE → Understand core principles
2. Load PROTOCOLS → Understand validation procedures (applies DOCTRINE)
3. Execute INSTRUCTIONS → Run workflows (applies PROTOCOLS + DOCTRINE)
```
